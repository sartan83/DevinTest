    ...
    <PanelShell eyebrow={p.eyebrow} compact bg="bright">
      <div className="flex flex-col gap-6 sm:gap-8 lg:gap-9">
                ...
                <div key={point.value} className="relative">
                  ...
                  >
                    <div className="flex items-baseline gap-1.5 sm:gap-2">
                      ...
                      {point.unit && (
                        <span className="text-[12px] font-light leading-none text-brand-ivory/70 sm:text-[13px] lg:text-[14.5px]">
                          {point.unit}
                        </span>
                      )}
                    </div>
                    <span className="text-[11px] uppercase tracking-[0.24em] text-brand-ivory/80 sm:text-[11.5px] lg:text-[12px]">
                      {point.label}
                    </span>
                  ...
                  {i < p.valueArc.points.length - 1 && (
                    <motion.span
                      ...
                      className="pointer-events-none absolute right-[-22px] top-[52%] hidden -translate-y-1/2 select-none text-[26px] font-light leading-none text-brand-ivory/45 sm:right-[-18px] lg:block"
                    >
                      →
                    </motion.span>
                  )}
        ...
        <motion.div
          ...
          className="flex flex-col gap-3 border-t border-brand-ivory/16 pt-4 sm:gap-2 sm:pt-5"
        >
          <span className="text-[11px] uppercase tracking-[0.28em] text-brand-orange-soft sm:text-[11.5px]">
            ROI logic
          </span>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-2 lg:gap-3">
            {p.logicBar.map((step, i) => (
              ...
              >
                <span className="text-[11px] uppercase tracking-[0.24em] text-brand-ivory/55 sm:text-[11.5px]">
                ...
                <div className="flex min-w-0 flex-col gap-0.5">
                  ...
                  <span className="text-[12px] leading-snug text-brand-ivory/75 sm:text-[12.5px]">
                    {step.result}
                  </span>
                </div>
                {i < p.logicBar.length - 1 && (
                  <span
                    aria-hidden
                    className="pointer-events-none absolute right-[-10px] top-[26%] hidden select-none text-[18px] font-light leading-none text-brand-ivory/40 sm:block"
                  >
                    →
                  </span>
                )}
        ...
        <motion.div
          ...
          className="flex flex-wrap items-baseline gap-x-5 gap-y-1.5 text-[11.5px] text-brand-ivory/75 sm:text-[12.5px]"
        >
          {p.scenarioLine.map((s, i) => {
            ...
            return (
              <span key={s.eff} className="flex items-baseline gap-1.5">
                ...
                <span className="text-brand-ivory/55">→</span>
                ...
                {i < p.scenarioLine.length - 1 && (
                  <span aria-hidden className="ml-3 text-brand-ivory/35">
                    ·
                  </span>
                )}
              </span>
        ...
        <div className="flex flex-col gap-1.5">
          <motion.p
            ...
            className="text-[11.5px] italic leading-snug text-brand-ivory/60 sm:text-[12px]"
          >
            {p.caveat}
          </motion.p>
          {p.pilotFooter && (
            <motion.p
              ...
              className="text-[11.5px] leading-snug text-brand-ivory/70 sm:text-[12px]"
            >
              {p.pilotFooter}
            </motion.p>
          )}
