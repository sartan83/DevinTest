  ...
  return (
    <PanelShell eyebrow={p.eyebrow} compact bg="deep">
      <div className="flex flex-col gap-4 sm:gap-5">
        ...
        <div className="flex flex-col gap-2 sm:gap-2.5">
          ...
          {p.subhead && (
            <motion.p
              ...
              className="max-w-2xl text-[12.5px] leading-relaxed text-brand-ivory/80 sm:text-[14px] lg:text-[15px]"
            >
              {p.subhead}
            </motion.p>
          )}
        ...
        <div className="grid gap-2.5 sm:gap-3 lg:grid-cols-3">
          {p.pillars.map((c, i) => (
            <motion.div
              ...
              className="flex flex-col gap-2.5 rounded-xl border border-brand-ivory/16 bg-brand-green-mid/25 px-3.5 py-3.5 sm:px-4 sm:py-4"
            >
              <div className="flex items-baseline justify-between gap-2 border-b border-brand-ivory/16 pb-2">
                <span className="text-[11px] uppercase tracking-[0.24em] text-brand-orange-soft sm:text-[11.5px]">
                  {String(i + 1).padStart(2, "0")}
                </span>
                <span className="font-display text-[11px] uppercase tracking-[0.2em] text-brand-ivory/60 sm:text-[11.5px]">
                  Pillar
                </span>
              </div>
              ...
              {c.bullets && c.bullets.length > 0 && (
                <ul className="flex flex-col gap-1 text-[12px] uppercase tracking-[0.18em] text-brand-ivory/80 sm:text-[12.5px]">
        ...
        <div className="flex flex-col gap-2 border-t border-brand-ivory/16 pt-3 sm:flex-row sm:items-baseline sm:justify-between sm:gap-5">
          {p.proofMetric && (
            <motion.div
              initial={{ opacity: 0, y: 6 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              ...
              <span className="text-[12px] leading-snug text-brand-ivory/80 sm:text-[12.5px]">
                {p.proofMetric.label}
              </span>
            </motion.div>
          )}
          {p.reviewReframe && (
            <motion.p
              ...
              className="max-w-2xl text-[12px] italic leading-snug text-brand-ivory/75 sm:text-[13px] lg:text-right"
            >
              {p.reviewReframe}
            </motion.p>
          )}
