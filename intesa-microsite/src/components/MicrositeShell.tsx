...
export function MicrositeShell() {
  ...
  // Translate horizontal wheel intent: let vertical wheel become horizontal scroll.
  useEffect(() => {
    const el = scrollerRef.current;
    if (!el) return;
  ...
  return (
    <div className="relative h-[100svh] w-screen overflow-hidden bg-brand-green text-brand-ivory">
      {/* Top bar */}
      <header className="pointer-events-none absolute inset-x-0 top-0 z-30 flex items-start justify-between gap-6 px-8 pt-6 sm:px-12">
        <div className="pointer-events-auto flex items-center gap-3">
          <LogoMark />
          <div className="flex flex-col leading-tight">
            <span className="text-[11px] uppercase tracking-[0.28em] text-brand-ivory/60">
              {intesa.brand.partner}
            </span>
            <span className="text-sm font-medium text-brand-ivory">
      ...
      <div
        ref={scrollerRef}
        className="no-scrollbar scroll-snap-x h-full w-full overflow-x-auto overflow-y-hidden"
      >
        <div className="flex h-full" style={{ width: `${TOTAL_PANELS * 100}vw` }}>
      ...
      {/* Bottom progress + nav */}
      <footer className="pointer-events-none absolute inset-x-0 bottom-0 z-30 flex items-end justify-between gap-6 px-8 pb-6 sm:px-12">
        <ProgressBar active={active} onSelect={goTo} />
        <div className="pointer-events-auto flex items-center gap-2 text-[11px] uppercase tracking-[0.28em] text-brand-ivory/50">
          <KeyHint k="←" />
          <KeyHint k="→" />
          <span>navigate</span>
        </div>
      ...
      <AnimatePresence>
        {active === 0 && (
          <motion.div
            ...
            className="pointer-events-none absolute bottom-20 left-1/2 z-20 -translate-x-1/2 text-[11px] uppercase tracking-[0.3em] text-brand-ivory/45"
          >
            Swipe, scroll, or press →
          </motion.div>
        )}
      </AnimatePresence>
    </div>
...
function LogoMark() {
  return (
    <div className="flex h-9 w-9 items-center justify-center rounded-[10px] bg-devin-gradient shadow-glow">
      <span className="font-display text-base font-semibold text-brand-ivory">D</span>
    </div>
  );
}
